...
class ReconPipelineTests(unittest.TestCase):
    ...
    def test_build_recon_pipeline_is_deterministic_for_same_input(self):
        ...
        self.assertEqual([step.name for step in first.steps], ["http_probe", "httpx_probe", "web_fingerprint", "whatweb_fingerprint", "dns", "ipintel", "tls_inspection", "sslyze", "rdap", "port_scan"])
    ...
    def test_domain_pipeline_order(self):
        ...
        self.assertEqual(
            [step.name for step in pipeline.steps],
            ["dns", "subfinder", "ipintel", "http_probe", "httpx_probe", "web_fingerprint", "whatweb_fingerprint", "tls_inspection", "sslyze", "rdap", "rpcinfo", "port_scan"],
        )
    ...
    def test_surface_profile_excludes_network_intel_but_keeps_bounded_port_scan(self):
        pipeline = build_recon_pipeline("example.com", params={"strategy": "surface"})

        self.assertEqual(
            [step.name for step in pipeline.steps],
            ["dns", "subfinder", "http_probe", "httpx_probe", "web_fingerprint", "whatweb_fingerprint", "tls_inspection", "sslyze", "rdap", "rpcinfo", "port_scan"],
        )
    ...
    def test_fast_is_a_distinct_bounded_profile(self):
        fast = build_recon_pipeline("example.com", params={"strategy": "fast"})

        self.assertIn("nmap", [step.tool for step in fast.steps])
