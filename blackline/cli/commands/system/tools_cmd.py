...
def handle_tools(argument: str = "", *, use_color: bool | None = None) -> bool:
    ...
    if query == "install" or query.startswith("install "):
        _, _, target = query.partition(" ")
        return handle_install(target, use_color=use_color)
    if query.startswith("uninstall"):
        _, _, target = query.partition(" ")
        return handle_uninstall(target, use_color=use_color)
    if query in {"", "all"}:
    ...
    if query == "recon":
        ...
        return True
    if " " in query:
        error("usage: tools [recon|<tool>|install <tool|all>|uninstall <tool|all>]", use_color=use_color)
        return False
...
def _render_tool_list(names: tuple[str, ...], *, title: str, use_color: bool | None) -> None:
    ...
    for name in names:
        ...
        status = recon_tool_status(tool) if tool else ("ready" if _installed_location(name, name) else "unavailable")
...
def _render_tool_details(name: str, *, use_color: bool | None) -> None:
    ...
    health = check_recon_tool(recon_tool) if recon_tool else None
    location = health.path if health else _installed_location(name, binary)
...
def _installed_location(name: str, binary: str) -> str:
    """Find a verified PATH candidate or Blackline's user-local build output."""
    resolution = Pathfinder().locate(name, executable=binary)
    if resolution.path:
...
def _installer_rows(name: str) -> list[tuple[str, str]]:
    ...
    rows.append(("install", f"tools install {name}"))
    return rows
